...
def mean_squared_error_state(xs_pred, xs):
    return 8*(xs - xs_pred).square().mean()

def mean_squared_error_measurement(xs_pred, xs, y_dim):

    return y_dim*(xs - xs_pred).square().mean()
